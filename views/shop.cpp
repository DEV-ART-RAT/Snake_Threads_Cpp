...
auto mensajeShop = [](int opc,myGame<nodeuserinfouser>* game, int size) { 
    ...
    messageSteep(w.ws_col);
    (opc==1)? messageLineREDEmoji(w.ws_col,"*  Vida Extra (っ◔◡◔)っ ❤ (200 monedas)  *", 8):              
    ...
    (opc==3)? messageLineREDEmoji(w.ws_col,"*  Potenciar Points x3 (͠≖ ͜ʖ͠≖)👌 (1000 monedas) *", 8):    
        messageLine(w.ws_col,"   Potenciar Points x3 (1000 monedas)  ");
    (opc==4)? messageLineREDEmoji(w.ws_col,"*  Velocidad x2 Masoquista (͠◉_◉᷅ ) (100 monedas) *", 8):    
        messageLine(w.ws_col,"   Velocidad x2 Masoquista (100 monedas)  ");
    (opc==5)? messageLineREDEmoji(w.ws_col,"*  Velocidad x3 Demente (ㆆ_ㆆ) (100 monedas) *", 8):    
 ...
 */
void boostVelocity(myGame<nodeuserinfouser>* game){
    //Checks if has the enough money
    if(game->user->info.coin >= 200){
        game->user->info.coin -= 200;
        game->snake.lifes++;
